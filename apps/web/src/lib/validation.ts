...
export const themeSchema = z.object({
  ...
  borderColor: z.string().regex(/^#[0-9A-F]{6}$/i, 'Border color must be a valid hex color'),
...
export const performanceSettingsSchema = z.object({
  ...
  cacheDuration: z.number().min(0, 'Cache duration must be non-negative'),
...
export const localeSchema = z.object({
  code: z.string().min(2, 'Locale code must be at least 2 characters').max(5, 'Locale code must be less than 5 characters'),
...
export const localizationConfigSchema = z.object({
  currentLocale: z.string().min(2, 'Current locale must be at least 2 characters'),
  fallbackLocale: z.string().min(2, 'Fallback locale must be at least 2 characters'),
  availableLocales: z.array(localeSchema).min(1, 'At least one locale must be available'),
...
// Web-specific validation schemas
...
export const webCampaignSchema = z.object({
  id: z.string().optional(),
  ...
  targetSegments: z.array(z.string()).min(1, 'At least one target segment must be selected'),
...
export const webTemplateSchema = z.object({
  id: z.string().optional(),
  ...
  approvedBy: z.string().optional(),
  approvalDate: z.date().optional(),
  usageCount: z.number().min(0, 'Usage count must be non-negative').default(0),
  isPublic: z.boolean().optional(),
  rating: z.number().min(0, 'Rating must be non-negative').max(5, 'Rating must be at most 5').optional(),
  reviews: z.number().min(0, 'Reviews count must be non-negative').optional(),
});
...
export function validateWebContact(contact: unknown): { isValid: boolean; data?: unknown; errors: string[] } {
  return validateWithSchema(contact, webContactSchema);
}

export function validateWebCampaign(campaign: unknown): { isValid: boolean; data?: unknown; errors: string[] } {
  return validateWithSchema(campaign, webCampaignSchema);
}

export function validateWebTemplate(template: unknown): { isValid: boolean; data?: unknown; errors: string[] } {
  return validateWithSchema(template, webTemplateSchema);
}

export function validateAppSettings(settings: unknown): { isValid: boolean; data?: unknown; errors: string[] } {
  return validateWithSchema(settings, appSettingsSchema);
}
...
export const schemas = {
  ...
  locale: localeSchema,
  localizationConfig: localizationConfigSchema,
  appSettings: appSettingsSchema,
  webContact: webContactSchema,
  webCampaign: webCampaignSchema,
  webTemplate: webTemplateSchema,
} as const;
