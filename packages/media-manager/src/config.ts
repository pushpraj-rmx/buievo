    ...
    case "gcs":
      return {
        ...
        gcs: {
          ...
          credentials: process.env.GCS_CREDENTIALS ? JSON.parse(process.env.GCS_CREDENTIALS) : undefined, // Optional: direct credentials
        }
      };
...
export function getFallbackStorageConfig(): StorageConfig | undefined {
  ...
  switch (fallbackProvider) {
    case "whatsapp":
      config.whatsapp = {
        baseUrl: process.env.FALLBACK_META_API_BASE_URL || config.whatsapp?.baseUrl || "",
        accessToken: process.env.FALLBACK_ACCESS_TOKEN || config.whatsapp?.accessToken || "",
        phoneNumberId: process.env.FALLBACK_PHONE_NUMBER_ID || config.whatsapp?.phoneNumberId || "",
      };
      break;
    
    case "local":
      config.local = {
        uploadDir: process.env.FALLBACK_LOCAL_UPLOAD_DIR || config.local?.uploadDir || "./uploads",
        baseUrl: process.env.FALLBACK_LOCAL_BASE_URL || config.local?.baseUrl || "http://localhost:3001/media",
      };
      break;
    ...
    case "s3":
      config.s3 = {
        ...
        region: process.env.FALLBACK_S3_REGION || config.s3?.region || "us-east-1",
        accessKeyId: process.env.FALLBACK_S3_ACCESS_KEY_ID || config.s3?.accessKeyId || "",
        secretAccessKey: process.env.FALLBACK_S3_SECRET_ACCESS_KEY || config.s3?.secretAccessKey || "",
    ...
    case "gcs":
      config.gcs = {
        ...
        projectId: process.env.FALLBACK_GCS_PROJECT_ID || config.gcs?.projectId || "",
        keyFilename: process.env.FALLBACK_GCS_KEY_FILENAME || config.gcs?.keyFilename,
        credentials: process.env.FALLBACK_GCS_CREDENTIALS ? JSON.parse(process.env.FALLBACK_GCS_CREDENTIALS) : config.gcs?.credentials,
      };
      break;
  }
...
export function validateStorageConfig(config: StorageConfig): string[] {
  ...
  switch (config.provider) {
    case "whatsapp":
      if (!config.whatsapp?.accessToken) errors.push("WhatsApp access token is required");
      if (!config.whatsapp?.phoneNumberId) errors.push("WhatsApp phone number ID is required");
      break;
    
    case "local":
      if (!config.local?.uploadDir) errors.push("Local upload directory is required");
